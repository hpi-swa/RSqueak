...
class TestBasic(BaseJITTest):
    ...
    def test_very_large_integer_mul(self, spy, tmpdir):
        ...
        """)
        self.assert_matches(traces[0].loop, """
        guard_not_invalidated(descr=<Guard0x248b8e8>)
        i91 = int_le(i82, 10000)
        guard_true(i91, descr=<Guard0x24b60b0>)
        p93 = call_r(ConstClass(fromint), i82, descr=<Callr 8 i EF=3>)
        guard_no_exception(descr=<Guard0x248b950>)
        p95 = call_r(ConstClass(rbigint.mul), p55, p93, descr=<Callr 8 rr EF=4>)
        guard_no_exception(descr=<Guard0x248b9b8>)
        i96 = getfield_gc_i(p95, descr=<FieldS rpython.rlib.rbigint.rbigint.inst_size 24 pure>)
        i98 = int_gt(i96, 2)
        guard_true(i98, descr=<Guard0x24b6140>)
        i99 = getfield_gc_i(p95, descr=<FieldS rpython.rlib.rbigint.rbigint.inst_sign 16 pure>)
        i101 = int_lt(i99, 0)
        guard_false(i101, descr=<Guard0x24b6188>)
        f103 = call_f(ConstClass(rbigint.log), p103, 256, descr=<Callf 8 ri EF=4>)
        guard_no_exception(descr=<Guard0x248ba88>)
        f106 = float_add(f103, 0.100000)
        f108 = call_f(ConstClass(floor), f106, descr=<Callf 8 f EF=0>)
        i111 = cast_float_to_int(f108)
        i113 = int_add(i111, 1)
        i115 = int_add(i82, 1)
        i117 = int_sub(i86, 1)
        setfield_gc(ConstPtr(ptr118), i117, descr=<FieldS rsqueakvm.interpreter.Interpreter.inst_interrupt_check_counter 24>)
        i120 = int_le(i117, 0)
        guard_false(i120, descr=<Guard0x248baf0>)
        jump(p0, p1, i2, p3, p4, p7, p8, p10, p13, p15, i115, p23, p25, p27, p29, p31, p33, p35, p37, p39, p41, p43, p55, i117, descr=TargetToken(38510944))
        """)
