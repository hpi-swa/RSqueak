...
class TestBasic(BaseJITTest):
    ...
    def test_large_integer_long_div(self, spy, tmpdir):
        ...
        if not IS_64BIT:
            ...
            self.assert_matches(traces[0].loop, """
            guard_not_invalidated(descr=<Guard0x7f11e3891c90>)
            i71 = int_le(i62, 10000)
            guard_true(i71, descr=<Guard0x3318338>)
            i72 = int_is_zero(i62)
            guard_false(i72, descr=<Guard0x3318380>)
            i74 = int_eq(i62, -1)
            i75 = int_and(i55, i74)
            guard_false(i75, descr=<Guard0x33183c8>)
            i77 = call_i(ConstClass(ll_int_py_div__Signed_Signed), i52, i62, descr=<Calli 8 ii EF=0 OS=12>)
            i79 = int_add(i62, 1)
            i81 = int_sub(i66, 1)
            setfield_gc(ConstPtr(ptr82), i81, descr=<FieldS rsqueakvm.interpreter.Interpreter.inst_interrupt_check_counter 24>)
            i84 = int_le(i81, 0)
            guard_false(i84, descr=<Guard0x7f11e3891cf8>)
            jump(p0, p1, i2, p4, p5, p6, p8, p11, p13, i79, p21, p23, p25, p27, p29, p31, p33, p35, p37, p39, p41, i55, i52, i81, descr=TargetToken(56306128))
            """)
    ...
    def test_large_integer_div(self, spy, tmpdir):
        ...
        if not IS_64BIT:
            ...
            self.assert_matches(traces[0].loop, """
            guard_not_invalidated(descr=<Guard0x7fcb51071c90>)
            i71 = int_le(i62, 100000)
            guard_true(i71, descr=<Guard0x22b62a8>)
            i72 = int_is_zero(i62)
            guard_false(i72, descr=<Guard0x22b62f0>)
            i74 = int_eq(i62, -1)
            i75 = int_and(i55, i74)
            guard_false(i75, descr=<Guard0x22b6338>)
            i77 = call_i(ConstClass(ll_int_py_div__Signed_Signed), i52, i62, descr=<Calli 8 ii EF=0 OS=12>)
            i79 = int_add(i62, 1)
            i81 = int_sub(i66, 1)
            setfield_gc(ConstPtr(ptr82), i81, descr=<FieldS rsqueakvm.interpreter.Interpreter.inst_interrupt_check_counter 24>)
            i84 = int_le(i81, 0)
            guard_false(i84, descr=<Guard0x7fcb51071cf8>)
            jump(p0, p1, i2, p4, p5, p6, p8, p11, p13, i79, p21, p23, p25, p27, p29, p31, p33, p35, p37, p39, p41, i55, i52, i81, descr=TargetToken(39127504))
            """)
