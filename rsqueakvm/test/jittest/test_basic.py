...
class TestBasic(BaseJITTest):
    ...
    def test_large_integer_long_div(self, spy, tmpdir):
        ...
        if not IS_64BIT:
            ...
            guard_true(i76, descr=<Guard0x2264ba8>)
            i77 = int_is_zero(i67)
            guard_false(i77, descr=<Guard0x2264bf0>)
            i79 = int_eq(i67, -1)
            i80 = int_and(i60, i79)
            guard_false(i80, descr=<Guard0x2264c38>)
            i82 = call_i(ConstClass(ll_int_py_div__Signed_Signed), i57, i67, descr=<Calli 8 ii EF=0 OS=12>)
            i84 = int_add(i67, 1)
            i86 = int_sub(i71, 1)
            setfield_gc(ConstPtr(ptr87), i86, descr=<FieldS rsqueakvm.interpreter.Interpreter.inst_interrupt_check_counter 24>)
            i89 = int_le(i86, 0)
            guard_false(i89, descr=<Guard0x2223408>)
            jump(p0, p3, p4, i5, p6, p8, p9, p10, p13, p15, i84, p23, p25, p27, p29, p31, p33, p35, p37, p39, p41, p43, i60, i57, i86, descr=TargetToken(38367728))
            """)
    ...
    def test_large_integer_div(self, spy, tmpdir):
        ...
        if not IS_64BIT:
            ...
            self.assert_matches(traces[0].loop, """
            guard_not_invalidated(descr=<Guard0x18053a0>)
            i76 = int_le(i67, 100000)
            guard_true(i76, descr=<Guard0x1860bf0>)
            i77 = int_is_zero(i67)
            guard_false(i77, descr=<Guard0x1860c38>)
            i79 = int_eq(i67, -1)
            i80 = int_and(i60, i79)
            guard_false(i80, descr=<Guard0x1860c80>)
            i82 = call_i(ConstClass(ll_int_py_div__Signed_Signed), i57, i67, descr=<Calli 8 ii EF=0 OS=12>)
            i84 = int_add(i67, 1)
            i86 = int_sub(i71, 1)
            setfield_gc(ConstPtr(ptr87), i86, descr=<FieldS rsqueakvm.interpreter.Interpreter.inst_interrupt_check_counter 24>)
            i89 = int_le(i86, 0)
            guard_false(i89, descr=<Guard0x1805408>)
            jump(p0, p3, p4, i5, p6, p8, p9, p10, p13, p15, i84, p23, p25, p27, p29, p31, p33, p35, p37, p39, p41, p43, i60, i57, i86, descr=TargetToken(27882048))
            """)
    ...
    def test_very_large_integer_mul(self, spy, tmpdir):
        ...
        if IS_64BIT: # difference: 64bit has no native_bytes is None check
            self.assert_matches(traces[0].loop, """
            guard_not_invalidated(descr=<Guard0x337b7b0>)
            i111 = int_le(i101, 10000)
            guard_true(i111, descr=<Guard0x33d4d10>)
            p113 = call_r(ConstClass(W_BytesObject.getrbigint), p13, p62, descr=<Callr 8 rr EF=4>)
            guard_no_exception(descr=<Guard0x337b818>)
            p115 = call_r(ConstClass(fromint), i101, descr=<Callr 8 i EF=3>)
            guard_no_exception(descr=<Guard0x337b880>)
            p117 = call_r(ConstClass(rbigint.mul), p113, p115, descr=<Callr 8 rr EF=4>)
            guard_no_exception(descr=<Guard0x337b8e8>)
            i118 = getfield_gc_i(p117, descr=<FieldS rpython.rlib.rbigint.rbigint.inst_size 24 pure>)
            i120 = int_gt(i118, 2)
            guard_true(i120, descr=<Guard0x33d4d58>)
            i121 = getfield_gc_i(p117, descr=<FieldS rpython.rlib.rbigint.rbigint.inst_sign 16 pure>)
            i123 = int_lt(i121, 0)
            guard_false(i123, descr=<Guard0x33d4da0>)
            i125 = call_i(ConstClass(rbigint.touint), p117, descr=<Calli 8 r EF=4>)
            p127 = guard_exception(ConstClass(OverflowError), descr=<Guard0x337b950>)
            f130 = call_f(ConstClass(rbigint.log), p117, 256, descr=<Callf 8 ri EF=4>)
            guard_no_exception(descr=<Guard0x337b9b8>)
            f132 = call_f(ConstClass(floor), f130, descr=<Callf 8 f EF=0>)
            i133 = cast_float_to_int(f132)
            i135 = int_add(i133, 1)
            p139 = call_r(ConstClass(rbigint.tobytes), p117, i135, ConstPtr(ptr137), 0, descr=<Callr 8 riri EF=4>)
            guard_no_exception(descr=<Guard0x337ba20>)
            i140 = strlen(p139)
            i142 = int_le(i140, 4)
            guard_false(i142, descr=<Guard0x33d4de8>)
            p143 = new_array_clear(i140, descr=<ArrayU 1>)
            p145 = call_r(ConstClass(ll_string2list__GcArray_CharLlT_rpy_stringPtr), p139, descr=<Callr 8 r EF=5>)
            guard_no_exception(descr=<Guard0x337ba88>)
            i147 = int_add(i101, 1)
            i149 = int_sub(i105, 1)
            setfield_gc(ConstPtr(ptr150), i149, descr=<FieldS rsqueakvm.interpreter.Interpreter.inst_interrupt_check_counter 24>)
            i152 = int_le(i149, 0)
            guard_false(i152, descr=<Guard0x337baf0>)
            jump(p0, p3, p4, i5, p6, p8, p9, p10, p13, p15, i147, p23, p25, p27, p29, p31, p33, p35, p37, p39, p41, p43, p62, i149, descr=TargetToken(56480160))
            """)
        else:
            self.assert_matches(traces[0].loop, """
            guard_not_invalidated(descr=<Guard0xf5423488>)
            i112 = int_le(i102, 10000)
            guard_true(i112, descr=<Guard0xf542e6f4>)
            p114 = call_r(ConstClass(W_BytesObject.getrbigint), p13, p63, descr=<Callr 4 rr EF=4>)
            guard_no_exception(descr=<Guard0xf54234bc>)
            p116 = call_r(ConstClass(fromint), i102, descr=<Callr 4 i EF=3>)
            guard_no_exception(descr=<Guard0xf54234f0>)
            p118 = call_r(ConstClass(rbigint.mul), p114, p116, descr=<Callr 4 rr EF=4>)
            guard_no_exception(descr=<Guard0xf5423524>)
            i119 = getfield_gc_i(p118, descr=<FieldS rpython.rlib.rbigint.rbigint.inst_size 16 pure>)
            i121 = int_gt(i119, 2)
            guard_true(i121, descr=<Guard0xf542e718>)
            i122 = getfield_gc_i(p118, descr=<FieldS rpython.rlib.rbigint.rbigint.inst_sign 12 pure>)
            i124 = int_lt(i122, 0)
            guard_false(i124, descr=<Guard0xf542e73c>)
            i126 = call_i(ConstClass(rbigint.touint), p118, descr=<Calli 4 r EF=4>)
            p128 = guard_exception(ConstClass(OverflowError), descr=<Guard0xf5423558>)
            f131 = call_f(ConstClass(rbigint.log), p118, 256, descr=<Callf 8 ri EF=4>)
            guard_no_exception(descr=<Guard0xf542358c>)
            f133 = call_f(ConstClass(floor), f131, descr=<Callf 8 f EF=0>)
            i134 = cast_float_to_int(f133)
            i136 = int_add(i134, 1)
            p140 = call_r(ConstClass(rbigint.tobytes), p118, i136, ConstPtr(ptr138), 0, descr=<Callr 4 riri EF=4>)
            guard_no_exception(descr=<Guard0xf54235c0>)
            i141 = strlen(p140)
            i143 = int_le(i141, 4)
            guard_false(i143, descr=<Guard0xf542e760>)
            p144 = new_array_clear(i141, descr=<ArrayU 1>)
            p146 = call_r(ConstClass(ll_string2list__GcArray_CharLlT_rpy_stringPtr), p140, descr=<Callr 4 r EF=5>)
            guard_no_exception(descr=<Guard0xf54235f4>)
            i148 = int_add(i102, 1)
            i150 = int_sub(i106, 1)
            setfield_gc(ConstPtr(ptr151), i150, descr=<FieldS rsqueakvm.interpreter.Interpreter.inst_interrupt_check_counter 32>)
            i153 = int_le(i150, 0)
            guard_false(i153, descr=<Guard0xf5423628>)
            jump(p0, p1, i2, p3, p4, p7, p8, p10, p13, p15, i148, p23, p25, p27, p29, p31, p33, p35, p37, p39, p41, p43, p63, i150, descr=TargetToken(-180632000))
            """)
