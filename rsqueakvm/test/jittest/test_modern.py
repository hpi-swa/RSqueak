...
i := n' classified: 'none' withStamp: nil notifying: nil logSource: false.
    ...
    def test_nested_closure_loop_call(self, spy, tmpdir):
        ...
        """)
        self.assert_matches(traces[-2].setup, """
        p1 = getfield_gc_r(p0, descr=<FieldP rsqueakvm.storage_contexts.ContextPartShadow.inst__w_self 64>)
        i2 = getfield_gc_i(p0, descr=<FieldS rsqueakvm.storage_contexts.ContextPartShadow.inst__w_self_size 72>)
        i3 = getfield_gc_i(p0, descr=<FieldU rsqueakvm.storage_contexts.ContextPartShadow.inst__state_stackptr_pc 32>)
        p4 = getfield_gc_r(p0, descr=<FieldP rsqueakvm.storage_contexts.ContextPartShadow.inst__s_sender 24>)
        p5 = getfield_gc_r(p0, descr=<FieldP rsqueakvm.storage_contexts.ContextPartShadow.inst_closure 80>)
        p6 = getfield_gc_r(p0, descr=<FieldP rsqueakvm.storage_contexts.ContextPartShadow.inst__w_receiver 56>)
        p7 = getfield_gc_r(p0, descr=<FieldP rsqueakvm.storage_contexts.ContextPartShadow.inst__w_method 48>)
        p8 = getfield_gc_r(p0, descr=<FieldP rsqueakvm.storage_contexts.ContextPartShadow.inst_extra_data 88>)
        p9 = getfield_gc_r(p0, descr=<FieldP rsqueakvm.storage_contexts.ContextPartShadow.inst__temps_and_stack 40>)
        p11 = getarrayitem_gc_r(p9, 0, descr=<ArrayP 8>)
        p13 = getarrayitem_gc_r(p9, 1, descr=<ArrayP 8>)
        p15 = getarrayitem_gc_r(p9, 2, descr=<ArrayP 8>)
        p17 = getarrayitem_gc_r(p9, 3, descr=<ArrayP 8>)
        p19 = getarrayitem_gc_r(p9, 4, descr=<ArrayP 8>)
        p21 = getarrayitem_gc_r(p9, 5, descr=<ArrayP 8>)
        p23 = getarrayitem_gc_r(p9, 6, descr=<ArrayP 8>)
        p25 = getarrayitem_gc_r(p9, 7, descr=<ArrayP 8>)
        p27 = getarrayitem_gc_r(p9, 8, descr=<ArrayP 8>)
        p29 = getarrayitem_gc_r(p9, 9, descr=<ArrayP 8>)
        p31 = getarrayitem_gc_r(p9, 10, descr=<ArrayP 8>)
        p33 = getarrayitem_gc_r(p9, 11, descr=<ArrayP 8>)
        p35 = getarrayitem_gc_r(p9, 12, descr=<ArrayP 8>)
        p37 = getarrayitem_gc_r(p9, 13, descr=<ArrayP 8>)
        p39 = getarrayitem_gc_r(p9, 14, descr=<ArrayP 8>)
        p41 = getarrayitem_gc_r(p9, 15, descr=<ArrayP 8>)
        guard_value(i3, 1077936142, descr=<Guard0x4c97af0>)
        guard_not_invalidated(descr=<Guard0x6789388>)
        guard_value(p7, ConstPtr(ptr43), descr=<Guard0x4c97b58>)
        guard_nonnull_class(p11, 13592368, descr=<Guard0x4c97bc0>)
        p46 = getfield_gc_r(ConstPtr(ptr45), descr=<FieldP rsqueakvm.model.pointers.W_PointersObject.inst_strategy 32>)
        guard_value(p46, ConstPtr(ptr47), descr=<Guard0x4c97c28>)
        p49 = getfield_gc_r(ConstPtr(ptr48), descr=<FieldP rsqueakvm.model.pointers.W_PointersObject.inst_strategy 32>)
        guard_value(p49, ConstPtr(ptr50), descr=<Guard0x4c97c90>)
        guard_nonnull(p0, descr=<Guard0x67893d0>)
        p51 = force_token()
        enter_portal_frame(1, 0)
        i54 = getfield_gc_i(p11, descr=<FieldS rsqueakvm.model.numeric.W_SmallInteger.inst_value 8 pure>)
        i56 = int_lt(i54, 0)
        guard_false(i56, descr=<Guard0x6789418>)
        leave_portal_frame(1)
        guard_nonnull(p5, descr=<Guard0x6789460>)
        leave_portal_frame(1)
        p59 = force_token()
        setfield_gc(p0, p59, descr=<FieldP rsqueakvm.storage_contexts.ContextPartShadow.vable_token 16>)
        p60 = new_with_vtable(descr=<SizeDescr 16>)
        setfield_gc(p60, i54, descr=<FieldS rsqueakvm.interpreter.IntLocalReturn.inst__value 8 pure>)
        guard_not_forced_2(descr=<Guard0x9ab1280>)
        finish(p60, descr=<ExitFrameWithExceptionDescrRef object at 0xec2f40>)
        """)
        self.assert_matches(traces[-1].loop, """
