...
i := n' classified: 'none' withStamp: nil notifying: nil logSource: false.
    ...
    def test_global_class_access(self, spy, tmpdir):
        ...
        enter_portal_frame(4, 0),
        leave_portal_frame(4),
        guard_class(p153, ConstClass(W_PointersObject), descr=<Guard0xaf52454>)
    ...
    def test_interval_do(self, spy, tmpdir):
        ...
        self.assert_matches(traces[0].loop,
        """
        guard_not_invalidated(descr=<Guard0x4ccfa20>)
        i176 = int_lt(i66, i49)
        guard_true(i176, descr=<Guard0x67bef08>)
        p177 = getfield_gc_r(p6, descr=<FieldP rsqueakvm.model.pointers.W_PointersObject.inst_strategy 32>)
        guard_value(p177, ConstPtr(ptr178), descr=<Guard0x4ccfaf0>)
        p179 = getfield_gc_r(p6, descr=<FieldP rsqueakvm.model.pointers.W_PointersObject.inst__storage 24>)
        i181 = getarrayitem_gc_i(p179, 0, descr=<ArrayS 8>)
        i183 = int_eq(i181, 9223372036854775807)
        guard_false(i183, descr=<Guard0x67beec0>)
        i185 = getarrayitem_gc_i(p179, 2, descr=<ArrayS 8>)
        i187 = int_eq(i185, 9223372036854775807)
        guard_false(i187, descr=<Guard0x67bee30>)
        i188 = int_mul_ovf(i66, i185)
        guard_no_overflow(descr=<Guard0x67bede8>)
        i189 = int_add_ovf(i181, i188)
        guard_no_overflow(descr=<Guard0x67beda0>)
        i191 = int_add(i66, 1)
        p193 = getfield_gc_r(ConstPtr(ptr192), descr=<FieldP rsqueakvm.model.pointers.W_PointersObject.inst_strategy 32>)
        guard_value(p193, ConstPtr(ptr194), descr=<Guard0x4ccfa88>)
        p196 = getarrayitem_gc_r(p77, 0, descr=<ArrayP 8>)
        p198 = getarrayitem_gc_r(p77, 1, descr=<ArrayP 8>)
        p199 = force_token()
        enter_portal_frame(1, 0)
        guard_class(p196, ConstClass(W_LargeIntegerBig), descr=<Guard0x67bed58>)
        p203 = getfield_gc_r(p196, descr=<FieldP rsqueakvm.model.base.W_AbstractObjectWithClassReference.inst_w_class 24 pure>)
        guard_value(p203, ConstPtr(ptr204), descr=<Guard0x67bed10>)
        p206 = getfield_gc_r(ConstPtr(ptr205), descr=<FieldP rsqueakvm.model.pointers.W_PointersObject.inst_strategy 32>)
        guard_value(p206, ConstPtr(ptr207), descr=<Guard0x4ccf950>)
        p208 = force_token()
        enter_portal_frame(1, 0)
        p212 = getfield_gc_r(ConstPtr(ptr211), descr=<FieldP rsqueakvm.model.pointers.W_PointersObject.inst_strategy 32>)
        guard_value(p212, ConstPtr(ptr213), descr=<Guard0x4ccfc28>)
        p215 = getfield_gc_r(ConstPtr(ptr214), descr=<FieldP rsqueakvm.model.pointers.W_PointersObject.inst__storage 24>)
        p217 = getarrayitem_gc_r(p215, 1, descr=<ArrayP 8>)
        guard_nonnull_class(p217, ConstClass(W_PointersObject), descr=<Guard0x4ccfc90>)
        i219 = getfield_gc_i(p196, descr=<FieldS rsqueakvm.model.numeric.W_LargeIntegerBig.inst__exposed_size 40>)
        p220 = getfield_gc_r(p196, descr=<FieldP rsqueakvm.model.numeric.W_LargeIntegerBig.inst_value 48 pure>)
        i222 = cond_call_value_i(i219, ConstClass(calculate_exposed_size_for_big_int), p220, descr=<Calli 8 r EF=4>)
        guard_no_exception(descr=<Guard0x4ccfcf8>)
        p224 = getfield_gc_r(ConstPtr(ptr223), descr=<FieldP rsqueakvm.model.pointers.W_PointersObject.inst_strategy 32>)
        setfield_gc(p196, i222, descr=<FieldS rsqueakvm.model.numeric.W_LargeIntegerBig.inst__exposed_size 40>)
        guard_value(p224, ConstPtr(ptr225), descr=<Guard0x4ccfd60>)
        p226 = getfield_gc_r(p224, descr=<FieldP rsqueakvm.storage.AbstractCachingShadow.inst_version 32 pure>)
        guard_value(p226, ConstPtr(ptr227), descr=<Guard0x67befe0>)
        p228 = getfield_gc_r(p217, descr=<FieldP rsqueakvm.model.pointers.W_PointersObject.inst_strategy 32>)
        guard_value(p228, ConstPtr(ptr229), descr=<Guard0x4ccfdc8>)
        p230 = getfield_gc_r(p228, descr=<FieldP rsqueakvm.storage.AbstractStrategy.inst_w_class 8 pure>)
        guard_value(p230, ConstPtr(ptr231), descr=<Guard0x67bf028>)
        p233 = getfield_gc_r(ConstPtr(ptr232), descr=<FieldP rsqueakvm.model.pointers.W_PointersObject.inst_strategy 32>)
        guard_value(p233, ConstPtr(ptr234), descr=<Guard0x4ccfe30>)
        p235 = force_token()
        enter_portal_frame(1, 0)
        i239 = int_lt(i222, 0)
        guard_false(i239, descr=<Guard0x67bf070>)
        p240 = new_array_clear(i222, descr=<ArrayU 1>)
        p242 = getfield_gc_r(ConstPtr(ptr241), descr=<FieldP rsqueakvm.model.pointers.W_PointersObject.inst_strategy 32>)
        guard_value(p242, ConstPtr(ptr243), descr=<Guard0x4ccfe98>)
        leave_portal_frame(1)
        p245 = force_token()
        enter_portal_frame(1, 0)
        p249 = getfield_gc_r(ConstPtr(ptr248), descr=<FieldP rsqueakvm.model.pointers.W_PointersObject.inst_strategy 32>)
        guard_value(p249, ConstPtr(ptr250), descr=<Guard0x4ccff00>)
        p251 = force_token()
        enter_portal_frame(1, 0)
        leave_portal_frame(1)
        i256 = int_sub(i222, 1)
        i257 = int_le(i222, i256)
        guard_false(i257, descr=<Guard0x67bf0b8>)
        p258 = force_token()
        p259 = new_with_vtable(descr=<SizeDescr 64>)
        p260 = new_with_vtable(descr=<SizeDescr 8>)
        setfield_gc(p259, p260, descr=<FieldP rsqueakvm.model.variable.W_BytesObject.inst_version 48 pure>)
        setfield_gc(p259, ConstPtr(ptr261), descr=<FieldP rsqueakvm.model.base.W_AbstractObjectWithClassReference.inst_w_class 24 pure>)
        setfield_gc(p259, 9223372036854775807, descr=<FieldS rsqueakvm.model.base.W_AbstractObjectWithIdentityHash.inst_hash 8 pure>)
        setfield_gc(p0, p258, descr=<FieldP rsqueakvm.storage_contexts.ContextPartShadow.vable_token 16>)
        setfield_gc(p259, p240, descr=<FieldP rsqueakvm.model.variable.W_BytesObject.inst_bytes 40>)
        call_may_force_n(ConstClass(_replace_from_to_trampoline__v122___simple_call__function__), 0, i256, 0, p259, p196, descr=<Callv 0 iiirr EF=7>)
        guard_not_forced(descr=<Guard0x9aeb210>)
        guard_no_exception(descr=<Guard0x67bf100>)
        guard_not_invalidated(descr=<Guard0x67bf148>)
        leave_portal_frame(1)
        leave_portal_frame(1)
        guard_class(p198, ConstClass(W_PointersObject), descr=<Guard0x67bf190>)
        p269 = getfield_gc_r(p198, descr=<FieldP rsqueakvm.model.pointers.W_PointersObject.inst_strategy 32>)
        guard_value(p269, ConstPtr(ptr270), descr=<Guard0x4c50088>)
        p272 = getfield_gc_r(ConstPtr(ptr271), descr=<FieldP rsqueakvm.model.pointers.W_PointersObject.inst_strategy 32>)
        guard_value(p272, ConstPtr(ptr273), descr=<Guard0x4c50020>)
        p274 = getfield_gc_r(p198, descr=<FieldP rsqueakvm.model.pointers.W_PointersObject.inst__storage 24>)
        leave_portal_frame(1)
        ...
        setarrayitem_gc(p274, 0, p259, descr=<ArrayP 8>)
        i283 = int_le(i279, 0)
        guard_false(i283, descr=<Guard0x4ccff68>)
        jump(p0, p1, i2, p4, p5, p6, p8, p11, i189, i191, p17, p259, p25, p27, p29, p31, p33, p35, p37, p39, p41, p43, i49, p77, p72, descr=TargetToken(108868480))
        """)
